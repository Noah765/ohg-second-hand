...
type Body = { name: string, email: string, chats: { id: number; offer_title: string; offer_creator: string, chat_user: string; messages: { created_at: string, message: string, images: string[] | null; }[]; }[]; }[];

const transporter = createTransport({ host: 'ohg-monheim.eu', port: 587, auth: { user: 'noah.landgraf', pass: EMAIL_PASSWORD } });
...
export async function POST({ request }) {
    ...
    for (const { name, email, chats } of body) {
        ...
        if (messageCount === 1) {
            ...
            content = `Hallo ${name},<br><br>du hast eine neue Nachricht von ${otherUser} zum Angebot ${chat.offer_title} erhalten (<a href="${PUBLIC_SITE_URL}/chat/${chat.id}">zum Chat</a>):<br><br>----- (${formatTime(message.created_at)})<br><br>${message.message}`;
        ...
        await transporter.sendMail({ from: { address: 'noah.landgraf@ohg-monheim.eu', name: 'OHG Second Hand' }, to: email, subject, html: content });
    }
