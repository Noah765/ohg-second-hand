<script lang="ts">
	...
	export let offer: Exclude<PageData['offer'], null>;
	...
	async function submit() {
		...
		const body = {
			deleteAllReportsOffer: actions.includes('0') ? offer.id : undefined,
			deleteAllOfferReportsAccount: actions.includes('1') ? offer.creator.id : undefined,
			deleteOffer: actions.includes('2') ? offer.id : undefined,
			deleteAllOffersCreator: actions.includes('3') ? offer.creator.id : undefined,
			blockOfferCreator: actions.includes('4')
				? {
						id: offer.creator.id,
		...
		if (actions.includes('2') || actions.includes('3')) {
			goto('/shop');
			...
			alerts.update((oldAlerts) => [
				...oldAlerts,
				{
					...
					title: `Angebot${actions.includes('3') ? 'e' : ''} gelöscht`,
					description: `${actions.includes('2') ? 'Das Angebot wurde' : 'Die Angebote wurden'} erfolgreich gelöscht`
				}
			]);
		} else dispatch('close');
	}
...
<Modal>
	...
	<div class="mt-2 w-full text-xl">
		<Select
			...
			multiple
			multipleValueRegex={/^.+(?=3)|0-(?=[123])/g}
		>
			{#if !/[123]/.test(actions)}
				<OptionMultiple value="0">Alle Meldungen zum Angebot löschen</OptionMultiple>
			{/if}
			{#if !actions.includes('3')}
				<OptionMultiple value="1">Alle Meldungen zum Account löschen</OptionMultiple>
			{/if}
			{#if !actions.includes('3')}
				<OptionMultiple value="2">Angebot löschen</OptionMultiple>
			{/if}
			<OptionMultiple value="3">Alle Angebote vom Account löschen</OptionMultiple>
			<OptionMultiple value="4">Ersteller des Angebotes blockieren</OptionMultiple>
		</Select>
	</div>
	{#if actions.includes('4')}
		...
		<textarea
			...
			placeholder="Du bist aufgrund deines Angebotes „{offer.title}“ {blockOfferCreatorTimeType === 4
				? 'dauerhaft'
